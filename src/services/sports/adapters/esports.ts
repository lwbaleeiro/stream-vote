...
export async function getUpcomingGames(apiKey: string): Promise<UnifiedGame[]> {
    ...
    const sportConfig = SPORTS.find((sport) => sport.key === "esports");
    ...
    for (const league of sportConfig.leagues) {
        try {
            ...
            if (!response.ok) {
                console.error(`[E-Sports] API Error ${response.status} for league ${league.id}: ${await response.text()}`);
                continue;
            }
            ...
            const games = json.data.map((game) => {
                ...
                const estimatedEnd = new Date(startAt.getTime() + (3 * 60 * 60 * 1000)); // ~3 hours for esports
                const pollClosesAt = new Date(startAt.getTime() - (5 * 60 * 1000));

                return {
                    id: String(game.id),
                    sportKey: "esports" as const,
                    leagueId: league.id,
            ...
            console.error(`[E-Sports] Fetch failed for league ${league.id}:`, error);
        }
    }
...
export async function getGameResult(apiKey: string, gameId: string): Promise<GameResult | undefined> {
    ...
    for (const league of sportConfig.leagues) {
        ...
        console.log(`[E-Sports] Fetching game from: ${url}`);
        ...
        try {
            ...
            console.log(`[E-Sports] Success: ${gameId} status is ${game.status}`);
            ...
            console.error(`[E-Sports] Fetch failed for ${gameId} in ${league.id}:`, err);
        }
    }
    return undefined;
}
